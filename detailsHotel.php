...
if(isset($_GET['hotels'])) {
  ...
  $select = $conn->query("SELECT * FROM hotels WHERE id = '{$hotelsID}' and state=1");
  ...
  if(isset($_POST['checkin'])) {
    ...
    $numberOfroom = $_POST['roomi'];
        ...
        if($checkin >= $current_date){ // start if checkin
          if($checkout >= $checkin){
          $insert = "INSERT INTO `reservationshotels` (`id`, `name`, `checkin`, `checkout`, `phone`, `people`, `email`, `room`) VALUES (NULL, '$name', '$checkin', '$checkout', '$phone', '$people', '$email', '$numberOfroom')";
              ...
              $save = $conn->query($insert); 
                  if($save === true){
                    ...
                    $update = $conn->query("UPDATE hotels SET `rooms`='$newRooms' WHERE id = '{$hotelsID}' ");
                    header("Location: detailsHotel.php?hotels=$hotelsID");
...
if($checkResult){
   ...
   echo "<meta http-equiv='refresh' content='0; url=detailsHotel.php?hotels=$hotelsID'>";
...
<body>
     <!-- Room details -->
<div class="container">
    <?php while($hotels = mysqli_fetch_assoc($select)): ?>
       <div class="page-header">
         <h2 class="text-center"><?= $hotels['HotelsName']; ?></h2>
       </div>
...
<div class="container">
  <div class="mySlides">
    ...
    <img src="<?= $hotels['Photo1'] ?>" style="width:100%">
  </div>

  <div class="mySlides">
    <div class="numbertext">2 / 6</div>
    <img src="<?= $hotels['Photo2'] ?>" style="width:100%">
  </div>

  <div class="mySlides">
    <div class="numbertext">3 / 6</div>
    <img src="<?= $hotels['Photo3'] ?>" style="width:100%">
  </div>
    
  <div class="mySlides">
    <div class="numbertext">4 / 6</div>
    <img src="<?= $hotels['Photo4'] ?>" style="width:100%">
  </div>

  <div class="mySlides">
    <div class="numbertext">5 / 6</div>
    <img src="<?= $hotels['Photo5'] ?>" style="width:100%">
  </div>
    
  <div class="mySlides">
    <div class="numbertext">6 / 6</div>
    <img src="<?= $hotels['Photo6']?>" style="width:100%">
  </div>
  ...
  <div class="row">
    <div class="column">
      <img class="demo cursor" src="<?= $hotels['Photo1'] ?>" style="width:100%" onclick="currentSlide(1)">
    </div>
    <div class="column">
      <img class="demo cursor" src="<?= $hotels['Photo2'] ?>" style="width:100%" onclick="currentSlide(2)" >
    </div>
    <div class="column">
      <img class="demo cursor" src="<?= $hotels['Photo3'] ?>" style="width:100%" onclick="currentSlide(3)" >
    </div>
    <div class="column">
      <img class="demo cursor" src="<?= $hotels['Photo4'] ?>" style="width:100%" onclick="currentSlide(4)" >
    </div>
    <div class="column">
      <img class="demo cursor" src="<?= $hotels['Photo5'] ?>" style="width:100%" onclick="currentSlide(5)" >
    </div>    
    <div class="column">
      <img class="demo cursor" src="<?= $hotels['Photo6'] ?>" style="width:100%" onclick="currentSlide(6)" >
    </div>
  </div>
</div>
...
<br>
<br>
<iframe src="<?= $hotels['i3D']; ?>" 
width="1170" height="450" frameborder="0" style="border:0;" allowfullscreen="" aria-hidden="false" tabindex="0"></iframe>
<br>
<br>
         
<table>
<tr style="height:80px;">
    <td><p><b>Room Type: </b> <?= $hotels['type']; ?></p></td>
</tr>

<tr style="height:80px;">
    <td><p><b>Room Price (per night): </b> <?= $hotels['price']; ?></p></td>
</tr>

<tr style="height:80px;">
    <td><p><b>Available Rooms: </b> <?= $hotels['rooms']; ?></p></td>
</tr>

<tr style="height:80px; ">
    <td ><p style="width: 1170px;"><b>Room details: </b> <?= $hotels['details']; ?></p></td>
</tr>

<tr >
    <td><?=($hotels['rooms'] <= 0)?'<p class="text-danger">reservations have been closed on this room!</p>':'';?></td>
</tr>
...
<?php endwhile; ?>
  ...
  style= "margin-left:30%; font-family: Trebuchet MS, Arial, Helvetica, sans-serif; border-collapse: collapse; width:800px; background-color: #f2f2f2;">
         <form action="detailsHotel.php?hotels=<?=$hotelsID?>" method="POST">

  <tr style="height:80px; ">
    <td><label class="form-control-label">Full name:</label> </td>
  <td> <input type="text" class="form-control" name="fullname" style="margin-left:-100px; width: 450px;" required> </td>
  </tr>
  ...
  <td><input type="text" class="form-control" name="phone" placeholder="05******** " pattern="[0-9]{10}" style="margin-left:-100px; width: 450px;" required> </td>
  </tr>
  ...
  <td><input type="email" class="form-control" name="email" placeholder="email@hotmail.com" style="margin-left:-100px; width: 450px;" required></td>
  </tr>
 
 
     <tr style="height:80px;">
  <td colspan= 2><input type="submit" class="form-control btn btn-primary" onclick="location.href='payment1.php'" value="Make Reservation" name="checkin" style="margin-left:310px; width: 150px;"></td>
  </tr>
